...
class _SafeQueue(Queue):
    ...
    def __init__(self, max_size=0, ctx=None, pending_work_items=None,
                 thread_wakeup=None, reducers=None):
        self.qm_wakeup = thread_wakeup
...
class ProcessPoolExecutor(_base.Executor):
    ...
    def __init__(self, max_workers=None, job_reducers=None,
        ...
        Args:
            ...
            job_reducers, result_reducers: Custom reducers for pickling the
                given calls and results in the Executor. If only `job_reducers`
                is provided, `result_reducer` will use the same reducers
            timeout: A timeout in seconds after which idle workers exit. If a
                new job is submitted after the timeout, the executor will start
                enough new Python processes to make sure the pool of workers is
                full.
    ...
    def _start_queue_management_thread(self):
        if self._queue_management_thread is None:
            ...
            def weakref_cb(_, qm_wakeup=self._queue_management_thread_wakeup):
